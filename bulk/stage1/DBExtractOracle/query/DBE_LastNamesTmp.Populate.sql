DECLARE
    ...
    CURSOR FirstNames IS
        SELECT          TRIM(SUBSTR(Descripcion, 1, INSTR(DESCRIPCION, ',')))
            FROM        Autores
            WHERE       Descripcion NOT LIKE '[MUS] %' AND
                        INSTR(DESCRIPCION, ',') <> 0
            GROUP BY    TRIM(SUBSTR(Descripcion, 1, INSTR(DESCRIPCION, ',')));
BEGIN
    OPEN FirstNames;
    LOOP
        ...
        LOOP
            ...
            IF  NamePart IS NOT NULL THEN
                INSERT INTO DBE_LastNamesTmp(Last_Name) VALUES(UPPER(NamePart));
                COMMIT;
            END IF;
        END LOOP;
    END LOOP;
